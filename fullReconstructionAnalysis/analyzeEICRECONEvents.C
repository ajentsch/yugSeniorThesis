...
// Author: Alex Jentsch
//
// Date of last author update: 11/1/2024
//
//------------------------
...
void analyzeEICRECONEvents(){


	TString fileList = "inputFileList_DVMP_18x275_GeV_10_21_2024.list";
	TString outputName = "ePIC_DVMP_18x275_GeV_";

	TString date = "11_1_2024_";
	...
	while(getline(fileListStream, fileName) && iEvent < 150000){
		...
		TTreeReaderArray<float> truth_px_array = {tree_reader, "MCParticlesHeadOnFrameNoBeamFX.momentum.x"};
        TTreeReaderArray<float> truth_py_array = {tree_reader, "MCParticlesHeadOnFrameNoBeamFX.momentum.y"};
        TTreeReaderArray<float> truth_pz_array = {tree_reader, "MCParticlesHeadOnFrameNoBeamFX.momentum.z"};
		...
		while (tree_reader.Next()) {
			...
			bool hasHighXLProton = false;

			//cout << "Event has " << truth_px_array.GetSize() << " MC particles..." << endl;
			...
			for(int imc = 0; imc < truth_px_array.GetSize(); imc++){
				...
				if(truth_pdg_array[imc] == 2212 && truth_genStatus_array[imc] == 1){ //only checking protons here
				
					if(mctrk.Mag() < 0.0){ continue; }

					hasHighXLProton = true;
			...
			for(int iRecoTrk = 0; iRecoTrk < reco_track_x.GetSize(); iRecoTrk++){
				...
				if(reco_track_PDG[iRecoTrk] != 0 || prec_reco_tracks.Eta() < 4.0){ continue; } //looking for B0 protons
